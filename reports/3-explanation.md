...
4. `apply_filters(...)` — filter "meaningful & non-trivial":
   ...
   - **penghapusan tautologi** via `CORRELATED_GROUPS` (rule yang kedua sisinya dari
     ...
     atau `derived_dwelling_category` ↔ `construction_method`/`total_units`) → **1.766 rule
     terhapus**.
...
**Ekspor:**
- `3-association-rules.csv` — seluruh 4.090 rule (ranked).
